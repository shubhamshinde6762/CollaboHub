...
const InviteRow = (props) => {
    ...
    const acceptHandler = async() => {
        try {
            const response = await axios.post("http://192.168.234.48:5000/api/v1/acceptInvite", {
    ...
    const rejectHandler = async() => {
        try {
            const response = await axios.post("http://192.168.234.48:5000/api/v1/rejectInvite", {
              "email": props.user.data.email,
              "projectId":props.data["_id"],
            });
